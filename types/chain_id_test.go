...
func TestIritaParseChainID(t *testing.T) {
	testCases := []struct {
		name     string
		chainID  string
		expError bool
		...
		{
			"valid chain-id, single digit", "irita-1", false, big.NewInt(1),
		},
		{
			"valid chain-id, multiple digits", "irita-10086", false, big.NewInt(10086),
		},
		{
			"invalid chain-id, double dash", "aragonchain-1-1", true, nil,
		},
	...
	for _, tc := range testCases {
		chainIDEpoch, err := IritaParseChainID(tc.chainID)
		if tc.expError {
			require.Error(t, err, tc.name)
			require.Nil(t, chainIDEpoch)

			require.False(t, IsValidChainIDForIrita(tc.chainID), tc.name)
		} else {
			require.NoError(t, err, tc.name)
			require.Equal(t, tc.expInt, chainIDEpoch, tc.name)
			require.True(t, IsValidChainIDForIrita(tc.chainID))
		}
	}
}
